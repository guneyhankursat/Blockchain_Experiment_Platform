...
class TestWallet extends abstract_signer_1.Signer {
    ...
    static random(provider, network = dist_1.bsv.Networks.testnet) {
        return new TestWallet(dist_1.bsv.PrivateKey.fromRandom(network), provider);
    }
    enableSplitFeeTx(on) {
        this.splitFeeTx = on;
    }
    ...
    getBalance(address) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            address = address || (yield this.getDefaultAddress());
            let utxoManager = this._utxoManagers.get(address.toString());
            if (utxoManager) {
                return {
                    confirmed: utxoManager.balance(),
                    unconfirmed: 0,
                };
            }
...
class CacheableUtxoManager {
    ...
    balance() {
        return this.availableUtxos.reduce((r, utxo) => r + utxo.satoshis, 0);
    }
